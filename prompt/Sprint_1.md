...
## Sprint 1 核心目标（S1）
启动基建：完成 VS Code 插件壳子加载 Webview，后端网关启动，打通“插件 -> 后端 -> 第三方大模型（DeepSeek）”的流式 SSE 链路，实现“你好，世界”的问答返回。
...
|------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
| 后端	 | 1. 初始化项目：`mkdir ai-backend && cd ai-backend`，使用 FastAPI（Python）或 NestJS（TS）。<br>2. 配置 `.env` 文件（`PORT=3000`, `DEEPSEEK_API_KEY=xxx`, `JWT_SECRET=xxx`）。<br>3. 编写基础 `main.py` / `app.module.ts`，挂载全局异常过滤器。<br>4. 生成 OpenAPI（Swagger）文档路由（`/docs`）。 | 	服务启动后，浏览器访问 `http://localhost:3000/docs` 能看到 Swagger 页面。|
...
|角色	|具体任务（核心逻辑）|	验收标准|
|---|---|---|
|后端|	1. 替换 Mock 逻辑，编写 `deepseek_adapter.py`。<br>2. 使用 `httpx.AsyncClient` 异步请求 DeepSeek 官方 API。<br>3. 关键：实现后端 SSE 与 DeepSeek 官方 SSE 格式的透传转换（统一转成 OpenAI 标准格式），并增加 `request_id` 用于后续链路追踪。|	后端日志显示成功拿到 DeepSeek 返回的 `data: [DONE]`，且没有报错。|
|插件|	1. 在 Extension Host 侧，使用 `Node.js` 原生 `fetch` 请求后端 `/v1/chat/completions`。<br>2. 利用 `ReadableStream` 解析 SSE 数据块，通过 `postMessage` 将增量文本（`delta.content`）逐字推给 Webview。3. Webview 使用 `setState` 实现打字机效果（逐字追加到消息气泡）。|	插件中输入 `"Hello"`，界面上 1-2 秒后开始逐个蹦出 DeepSeek 返回的文字。|
...
|插件	|1. 利用 `vscode.ExtensionContext.globalState` 存储用户的 API Key 和 模型选择（如 deepseek-v3）。<br>2. 在 Webview 加载时，先检查 `globalState` 中是否有 Key，若无则弹出一个输入框（`vscode.window.showInputBox`）。<br>3. 将用户输入的 Key 发送给后端换取 JWT，并存回 `globalState`（下次启动免登录）。	|关闭 VS Code 再打开，插件能自动读取 Key 并显示“已登录”状态，无需反复输入。|
...
### 🔧 研发环境准备清单（S1 启动会必做）
- 后端：申请 DeepSeek 或 OpenAI 的 API Key（建议准备 2 个测试账号防限流）。
